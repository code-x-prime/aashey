...
export function ProductForm({
  ...
  // Track whether admin manually edited each SEO field (prevents auto-fill override)
  const seoEditedRef = useRef({ metaTitle: false, metaDescription: false, keywords: false });
  ...
  // Move image to a different position (drag-to-reorder helper)
  const moveImage = (fromIndex: number, toIndex: number) => {
    if (toIndex < 0 || toIndex >= imagePreviews.length || fromIndex === toIndex) return;
    ...
    const withPrimary = newPreviews.map((p, i) => ({ ...p, isPrimary: i === 0 }));
    setImagePreviews(withPrimary);

    // Also reorder imageFiles array (only applies to new/local files)
    // Count new (non-server) images before each position to find file index
    const getFileIndex = (previews: typeof imagePreviews, upTo: number) =>
      previews.slice(0, upTo).filter((p) => !p.id).length;

    const fromIsNew = !imagePreviews[fromIndex].id;
    const toIsNew = !imagePreviews[toIndex].id;

    if (fromIsNew || toIsNew) {
      const fromFileIdx = getFileIndex(imagePreviews, fromIndex);
      const toFileIdx = getFileIndex(imagePreviews, toIndex);

      if (fromIsNew && toIsNew && fromFileIdx !== toFileIdx) {
        const newFiles = [...imageFiles];
        const [removed] = newFiles.splice(fromFileIdx, 1);
        newFiles.splice(toFileIdx, 0, removed);
        setImageFiles(newFiles);
      }
    }
  };

  // Drag event handlers for image reordering
      ...
      const fetchProductDetails = async () => {
        try {
          ...
          if (response.data.success) {
            ...
            if (productData.metaDescription) seoEditedRef.current.metaDescription = true;
            if (productData.keywords) seoEditedRef.current.keywords = true;
  ...
  const handleChange = (
    ...
    if (type === "checkbox") {
      ...
      // Track manual edits to SEO fields
      if (["metaTitle", "metaDescription", "keywords"].includes(name)) {
        seoEditedRef.current[name as keyof typeof seoEditedRef.current] = true;
      }

      setProduct((prev) => {
        const next = { ...prev, [name]: value };
        // Auto-fill SEO fields when product name changes (if not manually edited)
        if (name === "name") {
          if (!seoEditedRef.current.metaTitle) {
            next.metaTitle = value;
          }
          if (!seoEditedRef.current.keywords) {
            next.keywords = value;
          }
        }
        return next;
  ...
  // Generate all SEO fields from current product data
  const generateSEO = () => {
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = editorContent || product.description || "";
    const plainText = (tempDiv.textContent || tempDiv.innerText || "").replace(/\s+/g, " ").trim();
    const metaDesc = plainText.length > 157 ? plainText.substring(0, 157) + "..." : plainText;

    setProduct((prev) => ({
      ...prev,
      metaTitle: prev.name || prev.metaTitle,
      metaDescription: metaDesc || prev.metaDescription,
      keywords: prev.keywords || prev.name,
    }));
    // Mark as edited after manual generation
    seoEditedRef.current = { metaTitle: true, metaDescription: true, keywords: true };
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!hasVariants && imageFiles.length > 0) {
        // Primary image is always the first one (index 0) since drag-to-reorder
        formData.append("primaryImageIndex", "0");

        // Append each image file in current order
        imageFiles.forEach((file) => {
          formData.append("images", file);
        });
      } else if (hasVariants) {
        console.log(`📸 Skipping product images for variant product - will use variant-specific images`);
      }
      ...
      if (response.data.success) {
        ...
        // For edit mode: if existing images were reordered, persist the new order
        if (mode === "edit" && savedProductId) {
          const existingImages = imagePreviews.filter((p) => p.id);
          if (existingImages.length > 1) {
            try {
              const imageOrders = existingImages.map((p) => ({
                imageId: p.id,
                order: imagePreviews.indexOf(p),
              }));
              await api.patch(`/admin/products/${savedProductId}/images/reorder`, { imageOrders });
            } catch {
              // Non-critical: don't fail the whole save if reorder fails
            }
          }
        }
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      {imagePreviews.length > 1 && (
                        <p className="text-xs text-muted-foreground mt-0.5">Drag images to reorder · First image is primary</p>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {imagePreviews.map((preview, index) => (
                      ...
                      >
                        <div
                          className={`relative h-36 rounded-md overflow-hidden border-2 ${
                            index === 0
                        ...
                        >
                          ...
                          {index === 0 && (
                            <span className="absolute top-1 left-1 bg-primary text-white text-xs py-0.5 px-2 rounded-full">
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 flex gap-1">
                          {index !== 0 && (
                            <Button
                              ...
                              variant="outline"
                              size="icon"
                              className="h-6 w-6 bg-white hover:bg-primary hover:text-white shadow"
                              title="Make primary (move to first)"
                          ...
                          <Button
                            ...
                            variant="outline"
                            size="icon"
                            className="h-6 w-6 bg-white hover:bg-destructive hover:text-white shadow"
                            onClick={() => removeImage(index)}
                          >
                            <Trash2 className="h-3 w-3" />
                          </Button>
                        </div>
                        {/* Left/right arrows for precise ordering */}
                        {imagePreviews.length > 1 && (
          ...
          <div className="space-y-4 rounded-lg border p-4 bg-gray-50">
            ...
            {(!product.metaTitle && !product.metaDescription && !product.keywords) && (
              <div className="bg-amber-50 border border-amber-200 rounded-md px-3 py-2 text-xs text-amber-700">
                💡 SEO fields are auto-filled from the product name. Edit them manually or click "Auto-fill from Product" to regenerate.
              </div>
            )}
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs ${product.metaTitle && product.metaTitle.length > 60 ? "text-orange-500" : "text-muted-foreground"}`}>
                ...
                <Input
                  ...
                  onChange={handleChange}
                  placeholder={product.name || t("products.form.seo.title_placeholder")}
                />
                <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs ${product.metaDescription && product.metaDescription.length > 160 ? "text-destructive" : "text-muted-foreground"}`}>
                ...
                <Textarea
                  ...
                  rows={3}
                />
                {product.metaDescription && product.metaDescription.length > 160 && (
                  <span className="text-destructive text-xs">⚠️ {t("products.form.seo.too_long")}</span>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={handleChange}
                  placeholder={product.name || t("products.form.seo.keywords_placeholder")}
                />
                <p className="text-xs text-muted-foreground">
