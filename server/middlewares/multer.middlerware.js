...
// skipResize: true = upload original without compression (for product admin uploads)
export const processAndUploadImage = async (file, subfolder = "images", skipResize = false) => {
  try {
    console.log(`🔧 Processing image: ${file.originalname} (skipResize: ${skipResize})`);
    console.log(
    ...
    if (!skipResize) {
      ...
      uploadBuffer = await sharp(buffer)
        ...
        .toBuffer();
      console.log(`🔧 Sharp processing complete. Processed size: ${uploadBuffer.length} bytes`);
