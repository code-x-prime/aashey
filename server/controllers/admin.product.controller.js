    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      try {
        if (req.body.subCategoryIds) {
          parsedSubCategoryIds = JSON.parse(req.body.subCategoryIds);
        }
      } catch (e) {
...
export const updateProduct = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.subCategoryIds !== undefined) {
    ...
    try {
      parsedSubCategoryIds = JSON.parse(req.body.subCategoryIds);
      if (!Array.isArray(parsedSubCategoryIds)) parsedSubCategoryIds = [];
    } catch (e) {
      parsedSubCategoryIds = [];
    }
...
export const uploadProductImage = asyncHandler(async (req, res, next) => {
  ...
  try {
    // Process and upload image to S3 (no compression for product images)
    const imageUrl = await processAndUploadImage(
      req.file,
      `products/${productId}`,
      true
...
export const uploadVariantImage = asyncHandler(async (req, res, next) => {
  ...
  try {
    // Upload to S3 without compression for variant images
    const imageUrl = await processAndUploadImage(
      req.file,
      `variants/${variantId}`,
      true
...
export const reorderProductImages = asyncHandler(async (req, res) => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      // Ensure isPrimary aligns with order 0
      const imageAtOrderZero = imageOrders.find((io) => io.order === 0);
      if (imageAtOrderZero) {
        await tx.productImage.updateMany({
          where: { productId, id: { not: imageAtOrderZero.imageId } },
          data: { isPrimary: false },
        });
        await tx.productImage.update({
          where: { id: imageAtOrderZero.imageId },
    ...
    throw new ApiError(500, `Failed to reorder product images: ${error.message}`);
  }
});
