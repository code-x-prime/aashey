...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - aashey",
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - aashey",
...
export const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  // Find order with all needed relations
  const order = await prisma.order.findFirst({
    where: {
      id: orderId,
      userId: req.user.id,
  ...
  // Run DB updates in a transaction
  await prisma.$transaction(async (tx) => {
    ...
    // 2. Restock inventory for each item
    for (const item of order.items) {
      ...
      await tx.inventoryLog.create({
        data: {
          ...
          previousQuantity: item.variant?.quantity || 0,
          newQuantity: (item.variant?.quantity || 0) + item.quantity,
          createdBy: req.user.id,
        },
      });
    }

    // 3. Mark payment as refunded if applicable
    if (order.razorpayPayment && order.paymentStatus === "SUCCESS") {
  ...
  // 4. Cancel Shiprocket order if Shiprocket is enabled and order was synced (non-blocking)
  if (order.shiprocketOrderId) {
    import("../utils/shiprocket.js")
      .then(async ({ cancelShiprocketOrder, getShiprocketSettings }) => {
        try {
          ...
          console.error("Shiprocket cancel error (non-critical):", err.message);
        }
      })
      .catch((err) => console.error("Shiprocket import error:", err));
  }
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - aashey",
...
export const uploadReviewImages = asyncHandler(async (req, res, next) => {
  ...
  if (req.files.length > 3) {
    throw new ApiError(400, "Maximum 3 images allowed per review");
  }

  const imageUrls = [];
  for (const file of req.files) {
    const filename = await processAndUploadImage(file, `reviews/${req.user.id}`);
    imageUrls.push(getFileUrl(filename));
  }

  res.status(200).json(new ApiResponsive(200, { images: imageUrls }, "Images uploaded successfully"));
});
...
export const deleteReview = asyncHandler(async (req, res, next) => {
  ...
  // Delete review images from R2 (non-blocking)
  if (review.images && review.images.length > 0) {
    Promise.allSettled(review.images.map((img) => deleteFromS3(img))).catch(() => {});
  }
