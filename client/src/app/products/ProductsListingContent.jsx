...
export default function ProductsListingContent() {
    ...
    return (
        <div id="products-main" className="min-h-screen bg-[#FDF6E3]">
            <div className="mx-auto px-4 md:px-8 lg:px-16 xl:px-24 py-6 md:py-10">
                <div className="relative w-full h-[200px] md:h-[300px] mb-8 md:mb-10 rounded-2xl overflow-hidden border border-[#C9933A]/20 shadow-lg">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                            <p className="font-sans text-[13px] text-[#6B4423]">
                                {loading && !products.length ? <span className="inline-block h-4 w-32 bg-[#C9933A]/15 rounded animate-pulse" /> : <><span className="font-semibold text-[#3F1F00]">{products.length}</span> of <span className="font-semibold text-[#3F1F00]">{pagination.total || 0}</span> products</>}
                                {loading && products.length > 0 && <RiLoader4Line className="inline-block w-3.5 h-3.5 text-[#C9933A] animate-spin ml-2" />}
                            </p>
                            <div className="flex items-center gap-2">
                                <div className="flex items-center bg-white rounded-xl p-1 border border-[#C9933A]/15 gap-0.5">
                                    <button onClick={() => handleViewChange("grid")} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${viewMode === "grid" ? "bg-[#3F1F00] text-[#FDF6E3]" : "text-[#8B6040] hover:text-[#3F1F00]"}`}><RiGridLine className="w-4 h-4" /></button>
                                    <button onClick={() => handleViewChange("list")} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${viewMode === "list" ? "bg-[#3F1F00] text-[#FDF6E3]" : "text-[#8B6040] hover:text-[#3F1F00]"}`}><RiListUnordered className="w-4 h-4" /></button>
                                </div>
                                <div className="flex items-center bg-white rounded-xl border border-[#C9933A]/15 overflow-hidden">
                                    <span className="px-3 font-sans text-[11px] font-semibold tracking-widest text-[#8B6040] uppercase bg-[#FDF6E3]/60 border-r border-[#C9933A]/15 py-2.5 hidden sm:block">Sort</span>
                                    <select value={sortValue} onChange={handleSortChange} disabled={loading} className="px-3 py-2.5 font-sans text-[13px] text-[#3F1F00] bg-transparent outline-none cursor-pointer pr-8 appearance-none">
                        ...
                        {loading && products.length === 0 ? (
                            ...
                            <div className="flex flex-col gap-3">
                                {products.map((p) => <ProductListCard key={p.id} product={p} />)}
                            </div>
                        )}
                        {pagination.pages > 1 && (
                            <div className="flex justify-center items-center gap-1.5 mt-10">
                                <button onClick={() => handlePageChange(pagination.page - 1)} disabled={pagination.page === 1 || loading} className="w-9 h-9 rounded-xl border border-[#C9933A]/25 flex items-center justify-center text-[#3F1F00] hover:bg-[#FDF6E3] disabled:opacity-30 transition-all"><RiArrowDownSLine className="w-4 h-4 rotate-90" /></button>
                                {[...Array(pagination.pages)].map((_, i) => {
                                    const page = i + 1;
                                    const show = page === 1 || page === pagination.pages || (page >= pagination.page - 1 && page <= pagination.page + 1);
                                    if (!show) return null;
                                    return <button key={page} onClick={() => handlePageChange(page)} disabled={loading} className={`w-9 h-9 rounded-xl font-sans text-[13px] font-medium transition-all ${pagination.page === page ? "bg-[#3F1F00] text-[#FDF6E3]" : "border border-[#C9933A]/25"}`}>{page}</button>;
                                })}
                                <button onClick={() => handlePageChange(pagination.page + 1)} disabled={pagination.page === pagination.pages || loading} className="w-9 h-9 rounded-xl border border-[#C9933A]/25 flex items-center justify-center text-[#3F1F00] hover:bg-[#FDF6E3] disabled:opacity-30 transition-all"><RiArrowUpSLine className="w-4 h-4 rotate-90" /></button>
                            </div>
                        )}
                    </div>
                </div>
