...
export default function ChangePasswordPage() {
    ...
    const PasswordInput = ({ name, label, field }) => (
        <div>
            <label htmlFor={name} className="block text-sm font-medium text-[#3F1F00] mb-1">{label}</label>
            <div className="relative">
                ...
                <button type="button" className="absolute inset-y-0 right-0 pr-3 flex items-center text-[#7A4E2D] hover:text-[#5C3A1E]" onClick={() => togglePasswordVisibility(field)}>
                    <DynamicIcon name={showPassword[field] ? "EyeOff" : "Eye"} className="h-5 w-5" />
                </button>
            </div>
            {name === "newPassword" && <p className="text-xs text-[#6B4423] mt-1">Password must be at least 8 characters long</p>}
        </div>
    );
        ...
        <ClientOnly>
            <div className="container mx-auto py-10 px-4">
                <div className="flex items-center mb-8">
                    <Link href="/account" className="inline-flex items-center text-sm text-[#5C3A1E] hover:text-primary mr-4"><DynamicIcon name="ArrowLeft" className="mr-1 h-4 w-4" />Back to Account</Link>
