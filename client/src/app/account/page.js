                    ...
                    <div className="section-container py-8">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            {menuItems.map((item, index) => (
                                <Link key={index} href={item.href} className="card-premium p-4 hover:border-primary/20">
                                    ...
                                    <h3 className="font-semibold text-[#1A0A00] text-sm mb-0.5">{item.label}</h3>
                                    <p className="text-xs text-[#6B4423]">{item.desc}</p>
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="card-premium p-6 mb-6">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-lg font-bold text-[#1A0A00]">Profile Information</h2>
                                {!isEditing && (
                            ...
                            {isEditing ? (
                                <form onSubmit={handleSubmit}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label className="block text-sm font-medium text-[#3F1F00] mb-1.5">Full Name</label>
                                            <Input name="name" type="text" value={formData.name} onChange={handleChange} className="input-premium" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-[#3F1F00] mb-1.5">Phone Number</label>
                                            <Input name="phone" type="tel" value={formData.phone} onChange={handleChange} className="input-premium" />
                                        </div>
                                    </div>
                                    <div className="flex gap-2 justify-end">
                                        <Button type="submit" className="bg-[#3F1F00] hover:bg-[#C9933A] text-white font-semibold px-6 py-2 rounded-lg transition-all" disabled={isSubmitting}>
                                            {isSubmitting ? "Saving..." : "Save Changes"}
                                        </Button>
                                        <Button type="button" className="border-2 border-[#C9933A] text-[#3F1F00] hover:bg-[#FDF6E3] font-semibold px-6 py-2 rounded-lg transition-all" onClick={() => { setIsEditing(false); setPreview(null); setFormData({ name: user?.name || "", phone: user?.phone || "", profileImage: null }); }}>
                                            Cancel
                                        </Button>
                                    </div>
                                </form>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div>
                                        <p className="text-sm text-[#6B4423] mb-1">Full Name</p>
                                        <p className="font-medium text-[#1A0A00]">{user?.name || "Not provided"}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-[#6B4423] mb-1">Email</p>
                                        <p className="font-medium text-[#1A0A00]">{user?.email || "Not provided"}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-[#6B4423] mb-1">Phone</p>
                                        <p className="font-medium text-[#1A0A00]">{user?.phone || "Not provided"}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-[#6B4423] mb-1">Member Since</p>
                                        <p className="font-medium text-[#1A0A00]">{user?.createdAt ? formatDate(user.createdAt) : "Unknown"}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="card-premium p-6 mb-6">
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-lg font-bold text-[#1A0A00]">Saved Addresses</h2>
                                ...
                                <div className="grid gap-4">
                                    {addresses.slice(0, 2).map((address) => (
                                        <div key={address.id} className="border border-gray-100 rounded-xl p-4">
                                            ...
                                            <p className="font-medium text-[#1A0A00]">{address.name || user?.name}</p>
                                            <p className="text-sm text-[#5C3A1E]">{address.street}, {address.city}, {address.state} {address.postalCode}</p>
                                            <p className="text-sm text-[#5C3A1E]">{address.country}</p>
                                        </div>
                                    ))}
                                    {addresses.length > 2 && <p className="text-sm text-[#6B4423]">+ {addresses.length - 2} more addresses</p>}
                                </div>
                            ) : (
                                <div className="text-center py-8 border border-dashed border-gray-200 rounded-xl">
                                    <MapPin className="h-8 w-8 mx-auto text-[#8B6040] mb-2" />
                                    <p className="text-[#6B4423] text-sm">No addresses added yet</p>
                                    <Link href="/account/addresses">
                        ...
                        <div className="bg-gradient-to-r from-primary/5 via-orange-50 to-primary/5 rounded-2xl p-6 border border-primary/10">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="text-lg font-bold text-[#1A0A00]">Referral Program</h2>
                            </div>
                            <p className="text-[#5C3A1E] text-sm mb-4">Share your referral code and earn rewards!</p>
                            ...
                            {isLoadingReferral ? (
                                ...
                                <>
                                    <div className="bg-white rounded-xl p-4 border border-primary/20 mb-4">
                                        <label className="block text-xs font-medium text-[#6B4423] mb-2">Your Referral Code</label>
                                    ...
                                    {referralStats && (
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            <div className="bg-white rounded-xl p-3 text-center border">
                                                ...
                                                <p className="text-xs text-[#6B4423]">Total Referrals</p>
                                            </div>
                                            <div className="bg-white rounded-xl p-3 text-center border">
                                                <p className="text-xl font-bold text-green-600">{referralStats.completedReferrals || 0}</p>
                                                <p className="text-xs text-[#6B4423]">Completed</p>
                                            </div>
                                            <div className="bg-white rounded-xl p-3 text-center border">
