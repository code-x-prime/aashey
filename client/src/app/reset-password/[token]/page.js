...
const PasswordRequirement = ({ met, text }) => (
    <div className={`flex items-center gap-2 font-sans text-xs ${met ? "text-[#092D15]" : "text-red-500"}`}>
    ...
    if (isSuccess) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#FDF6E3] px-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-[0_4px_24px_rgba(63,31,0,0.08)] border border-[#C9933A]/15 p-8 text-center">
                    <div className="w-16 h-16 bg-[#092D15]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                        <CheckCircle className="w-8 h-8 text-[#092D15]" />
                    </div>
                    <h1 className="font-cormorant text-2xl font-semibold text-[#3F1F00] mb-2">Password Reset Successful!</h1>
                    <p className="font-sans text-sm text-[#5C3A1E] mb-6">
                        Your password has been changed successfully. You will be redirected to login page shortly.
                    </p>
                    <Link href="/auth?tab=login">
                        <Button className="w-full h-12 rounded-xl bg-[#3F1F00] hover:bg-[#C9933A] text-[#FDF6E3] font-sans font-semibold">Go to Login</Button>
                    </Link>
                </div>
            </div>
        );
                ...
                <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(63,31,0,0.08)] border border-[#C9933A]/15 p-8">
                    <div className="text-center mb-6">
                        <div className="w-14 h-14 bg-[#C9933A]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Lock className="w-7 h-7 text-[#C9933A]" />
                        </div>
                        <h1 className="font-cormorant text-2xl font-semibold text-[#3F1F00]">Reset Your Password</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        {/* New Password */}
                        <div>
                            <label className="block font-sans text-sm font-medium text-[#3F1F00] mb-1.5">
                                New Password
                            </label>
                            <div className="relative">
                                ...
                                <button
                                    ...
                                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#7A4E2D] hover:text-[#3F1F00]"
                                >
                            ...
                            {password.length > 0 && (
                                <div className="mt-3 p-3 bg-[#FDF6E3] rounded-xl space-y-1.5">
                                    <p className="font-sans text-xs font-medium text-[#3F1F00] mb-2">Password must have:</p>
                    ...
                    <div className="mt-6 text-center">
                        <Link
                            href="/auth?tab=login"
                            className="font-sans text-sm text-[#C9933A] hover:text-[#3F1F00] font-medium"
                        >
                            ← Back to Login
                        </Link>
                    </div>
