          ...
          <div className="p-6 flex flex-col">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-[#6B4423]">({product.reviewCount || 0} reviews)</span>
              </div>

              {/* Price */}
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-2xl font-bold text-primary">{formatCurrency(price)}</span>
                {hasDiscount && <span className="text-lg text-[#7A4E2D] line-through">{formatCurrency(originalPrice)}</span>}
              </div>

              {/* Short description */}
              {product.shortDescription && (
                <p className="text-[#5C3A1E] text-sm mb-4 line-clamp-3">{product.shortDescription}</p>
              )}
