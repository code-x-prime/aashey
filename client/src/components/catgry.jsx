...
export default function CategoriesCarousel() {
  ...
  return (
    <div className="w-full py-6 relative group max-w-7xl mx-auto">
      ...
      >
        <ChevronLeft className="h-5 w-5 text-[#3F1F00]" />
      </button>
      ...
      >
        {categories.map((category) => (
          ...
          >
            ...
            <span className="mt-2 text-xs lg:text-sm text-center font-medium text-[#3F1F00] group-hover/item:text-primary transition-colors line-clamp-2 max-w-[80px] lg:max-w-[100px]">
              {category.name}
            </span>
          </Link>
        ))}
      ...
      >
        <ChevronRight className="h-5 w-5 text-[#3F1F00]" />
      </button>
    </div>
  );
}
